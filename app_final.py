...
def executive_summary_tab():
    ...
    kpi_cols[0].markdown(create_kpi_card("Production", production_mt, "MT", trend_prod, trend_label), unsafe_allow_html=True)
    kpi_cols[1].markdown(create_kpi_card("Consumption", consumption_mt, "MT", trend_cons, trend_label), unsafe_allow_html=True)
    kpi_cols[4].markdown(create_kpi_card("Market Balance", balance_mt, "MT"), unsafe_allow_html=True)
    kpi_cols[2].markdown(create_kpi_card("Imports", imports_mt, "MT", trend_imp, trend_label), unsafe_allow_html=True)
    kpi_cols[3].markdown(create_kpi_card("Exports", exports_mt, "MT", trend_exp, trend_label), unsafe_allow_html=True)
    # ---- Charts ----
...
def product_analytics_tab():
    ...
    kpi_cols1[0].markdown(create_kpi_card("Production", production_mt, "MT", trend_prod, trend_label), unsafe_allow_html=True)
    kpi_cols1[1].markdown(create_kpi_card("Consumption", consumption_mt, "MT", trend_cons, trend_label), unsafe_allow_html=True)
    kpi_cols1[2].markdown(create_kpi_card("Imports", imports_mt, "MT", trend_imp, trend_label), unsafe_allow_html=True)
    kpi_cols1[3].markdown(create_kpi_card("Exports", exports_mt, "MT", trend_exp, trend_label), unsafe_allow_html=True)
    kpi_cols1[4].markdown(create_kpi_card("Avg Price", price_avg, "$/kg", trend_price, trend_label), unsafe_allow_html=True)
    ...
    fig.update_xaxes(title="Period")  # ✅ use formatted labels like Q1-2025, June 2025
    st.plotly_chart(fig, use_container_width=True)
...
def region_details_tab():
    ...
    kpi_cols1[2].markdown(create_kpi_card("Imports", imports_mt, "MT", trend_imp, trend_label), unsafe_allow_html=True)
    kpi_cols1[3].markdown(create_kpi_card("Exports", exports_mt, "MT", trend_exp, trend_label), unsafe_allow_html=True)
    kpi_cols1[4].markdown(create_kpi_card("Market Balance", balance_mt, "MT"), unsafe_allow_html=True)
    
    
        # Create a row with 5 columns to mirror the layout of the top row.
    # This ensures the new cards are centered relative to the whole group.
    kpi_cols2 = st.columns(5)
    # Place the cards in the middle two columns, leaving the first, middle, and last columns empty.
    kpi_cols2[0].markdown(create_kpi_card("Active Consignees", int(active_consignees), "count"), unsafe_allow_html=True)
    kpi_cols2[1].markdown(create_kpi_card("Active Suppliers", int(active_suppliers), "count"), unsafe_allow_html=True)

    # The empty columns are implicitly created and will take up space to center the two cards.
    # You don't need to explicitly add empty markdown for them.
        # --- Charts ---
    ...
    figp = px.bar(
        port_df, x="Port", y=["Inbound_Volume_MT", "Outbound_Volume_MT"],
